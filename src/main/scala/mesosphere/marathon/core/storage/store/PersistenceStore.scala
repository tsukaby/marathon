...
trait PersistenceStore[K, Category, Serialized] {
  ...
  /**
    ...
    * The source will hold all items of a complete backup.
    * A future is returned which succeeds, if the backup is restored completely.
    * @param source the backup items to restore
    * @return a future that returns if the backup is restored completely.
    */
  def restore(source: Source[BackupItem, NotUsed]): Future[Done]
}
